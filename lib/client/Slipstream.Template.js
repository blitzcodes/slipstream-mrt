		...
		var templateName = options.name,
			self = _.extend(options, {
				...
				get             : function () {
					...
					config.debug("template::get - name = " + templateName, 'client', 'init', 'template');
				...
				submitForm      : function (e, t) {
					...
					config.debug("template::submitForm - template.data " + _.stringify(data), 'client', 'template');
				...
				removeEntry     : function () {
					...
					config.debug("template::removeEntry - template.data " + _.stringify(data), 'client', 'template');

					if (Col.hasId()) {
						try {
							result = Meteor.call(config.name + "Delete", data, function (err, id) {
								if (err) {
									if (err.error === 302) // if the error is that the post already exists, take us there
										;//Meteor.Router.to('postPage', error.details)
									else
