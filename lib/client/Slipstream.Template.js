	...
	Slipstream.Template = function (config, col, router, render) {
		var getTemplate = function (type) {
			...
			if (config.debug.template === true && config.debug.init === true && Meteor.isClient)
				Meteor._debug("template::getTemplate = " + templateName);
...
//					Meteor._debug("template::submitForm - template.data " + JSON.stringify(t.data));
			...
			processTemplates       : function () {
				...
				if (config.debug.template === true && config.debug.init === true && Meteor.isClient)
					Meteor._debug("template::templateOptions = " + JSON.stringify(self.templateOptions));
				...
				if (config.debug.template === true && config.debug.init === true && Meteor.isClient)
					Meteor._debug("template::final templates created = " + _.keys(self.templates));
			}
		};
