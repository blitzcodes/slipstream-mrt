...
(function () {
	...
	Slipstream.CollectionSetup = function (config, columns) {
		...
		_.extend(self, {
			checkColumnPermissions : function (type) {
				keys =
					[
					];
			...
			Meteor.startup(function () {
				_.defaults(config.deny, {
					...
					insert : function (userId, post, fieldNames) {
						return (_.without(fieldNames, self.checkColumnPermissions('insert')).length > 0);
					},
					...
					update : function (userId, doc, fieldNames, modifier) {
						var fieldCheck = !!(_.without(fieldNames, self.checkColumnPermissions('update')).length > 0);
						if(!fieldCheck)
