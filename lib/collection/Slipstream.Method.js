...
(function () {
	...
	Slipstream.Method = function (list, options) {
		var self = {
			...
			post        : function (data) {
				...
				if (this.columns.hasOwnProperty('dateCreated')) {
					data = _.extend(data, {
				...
				this.insert(data, function (error, id) {
					if (id) {
						...
						Slipstream.Error.log("There was an error updating your " + this.name + " entry, please try again.");
