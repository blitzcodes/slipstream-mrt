	...
	Slipstream.Collection = function (config) {
		var columns = _.extend({
			...
			process              : function (data) {
				...
				//				if (config.debug.collection && Meteor.isClient)
				//					Slipstream.debug("collection::process - arr:before = " + JSON.stringify(arr));
				...
				if (_.isFunction(config.postProcess)) {
					Slipstream.debug("Should be running postProcess..");
					arr = config.postProcess(arr);
				}

				if (config.debug.collection && Meteor.isClient)
					Slipstream.debug("collection::process - arr = " + JSON.stringify(arr));
			...
			validate             : function (data) {
				...
				if (config.debug.collection && Meteor.isClient)
					Slipstream.debug("collection::validate - arr = " + JSON.stringify(arr) + " errors = "
						+ JSON.stringify(self.errors) + ", result = " + result);
