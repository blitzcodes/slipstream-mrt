...
(function () {
	...
	Slipstream.Collection = function (config) {
		Slipstream.groupCollapsed("Init Collection");
		...
		// followed by the resulting Mongo cursor to populate the columns
			...
			self = _.extend(col, {
				...
				loadWhere            : function (where) {
					Slipstream.groupCollapsed("Collection.loadWhere");
					var result = loadValues(self, col.findOne(where));
					Slipstream.groupEnd();
					return result;
				},
				loadByTemplate       : function (t) {
					Slipstream.groupCollapsed("Collection.loadByTemplate");
				...
				getInputs            : function () {
					Slipstream.groupCollapsed("Collection.getInputs() - Render Columns");
					_.each(self.columnNames(), function (key) { // this referring to one of the filed columnNames in the columnNames array
						Slipstream.groupCollapsed("Rendering - " + key);
				...
				getColumns           : function () {
					Slipstream.groupCollapsed("Collection.getColumns");
					_.each(self.columnNames(), function (key) { // this referring to one of the filed columnNames in the columnNames array
						columns[key].render();
					});
				...
				process              : function (data) {
					...
					Slipstream.groupCollapsed("Collection.process");
