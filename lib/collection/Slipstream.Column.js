					...
					_.each(self.validation, function (key) {
						...
						if (_.isString(key) && _.has(config.validation, key)) // Check to see if a string was passed, and that it is a valid key within the list of validation types
							...
							Slipstream.throwError(602,
								"Non-fatal Error: You have specified an invalid validation option. Please choose one of the predefined patterns, extend Slipstream and add your own custom label/regex patterns, or pass in a custom regex directly. Failed regex type: "
									+ key);
						if (regex)
