...
(function () {
    /*
     ### `Slipstream.error`
     > Type: Function, Throws: Meteor.Error

     #### Description
     >
     */
    Slipstream.throwError = function (code, message) {
        /*
        ...
        var checkCode = function checkCode(code) {
            ...
            if (_.has(errorCodes, code))
                result = errorCodes[code].summary;

            return result;
        };

        // If no message was sent, find the default error for the code
        message = message || checkCode(code);

        Slipstream.debug("throwError: code (" + code + ") " + message);
