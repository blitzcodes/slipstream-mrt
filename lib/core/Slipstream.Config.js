	...
	Slipstream.Config = function (config) {
		var defaults = Slipstream.defaults(),
			self = _.defaults({
				...
				referenceColumn  : config.referenceColumn ? _.trim(config.referenceColumn.toLowerCase()) : defaults.referenceColumn,
				sessionReference : 'current' + _.capitalize(config.name.toLowerCase()) + 'Id',
