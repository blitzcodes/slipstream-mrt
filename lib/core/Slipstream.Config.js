	...
	Slipstream.Config = function (config) {
		var defaults = Slipstream.defaults(),
			self = _.defaults({
				...
				debugging : _.isObject(config.debugging) ? _.defaults(config.debugging || {},
					defaults.debugging) : defaults.debugging,

				debug : function () {
					var checks = true;

					if (self.debugging.enabled) {
						var conditionChecks = arguments[0],
						//							message = arguments[1],
							args = Array.prototype.slice.call(arguments, 1);
						args[0] = "Slipstream::" + args[0];
						...
						_.each(conditionChecks, function (key) {
							...
							else if (_.has(self.debugging, key))
								checks = checks && self.debugging[key];
						});
